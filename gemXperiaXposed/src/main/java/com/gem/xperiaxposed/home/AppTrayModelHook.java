...
public class AppTrayModelHook extends ClassHook<AppTrayModel>
{
  ...
  public int before_getNumberOfDownloadedActivities()
  {
    ...
    for(ActivityItem item: packageHandler.getActivityItemSet())
      if(packageHandler.getActivityResolveInfo(item).isDownloadedApplication() && !isHidden(item))
        ++i;
    return i;
  }

  public int before_getNumberOfWorkActivities()
  {
    int i = 0;
    for(ActivityItem item: packageHandler.getActivityItemSet())
      if(packageHandler.isWorkActivityItem(item) && !isHidden(item))
        ++i;
    return i;
  }
